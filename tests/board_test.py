...
def test_empty_board():
    """
    Checks basic properties of the empty grid.
    """
    board = Board()
...
def test_str():
    """
    Tests string output of the grid.
    """
    board = Board()
    assert str(board) == "+--+--+--+--+--+\n" \
                         "|  |  |  |  |  |\n" \
...
def test_row():
    ...
    for i in range(Board.SIZE):
        assert all(board.row(i) == np.full((Board.SIZE,), Board.EMPTY_CELL))
    ...
    board.integrity_check()
    expected_row = np.asarray([Board.EMPTY_CELL, 1, 3, Board.EMPTY_CELL, 13])
...
def test_col():
    ...
    for i in range(Board.SIZE):
        assert all(board.col(i) == np.full((Board.SIZE,), Board.EMPTY_CELL))
    board.make_move((0, 1), 1)
    board.integrity_check()
    ...
    board.integrity_check()
    expected_col = np.asarray([1, 3, Board.EMPTY_CELL, Board.EMPTY_CELL, 13])
...
def test_possible_moves():
    """
    Tests single call of possible_moves() function on the grid.
    """
    board = Board()
