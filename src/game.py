...
the numbers for moves, adding/removing the players and letting them know
the current number, and evaluating the positions.
"""
...
class Game:
    """
    Class Game controls all card picking, asks players about
    next moves and assigns scores at the end.
    ...
    def take_next_card(self):
        ...
        :return: the number of the next card or None if the
        number of moves is sufficient to fill the board
        """
        if self.finished():
            return None
    ...
    def add_player(self, player: Player):
        """
        Adds the player to the game. If the game is in progress,
        throw ValueError
    ...
    def finished(self) -> bool:
        """
        :return: true if all moves have been played
        """
        return self.moves_played >= Board.SIZE ** 2 or self.moves_played >= len(self.available_cards)
    ...
    def start(self, verbose=False):
        """
        Simulates one game, for each round picks one card,
        lets players start their move and at the end computes
        final scores
        :param verbose: if True, prints information about game
        :return: list of final scores, the index corresponds
        to the index return by <add_player>
        """
        ...
        scores = [Evaluator.evaluate(player.get_board()) for player in self.players]
        if verbose:
